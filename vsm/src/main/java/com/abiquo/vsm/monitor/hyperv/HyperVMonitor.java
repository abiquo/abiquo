...
public class HyperVMonitor extends AbstractMonitor
{
    /** The logger. */
    private final static Logger LOGGER = LoggerFactory.getLogger(HyperVMonitor.class);
        ...
        throws MonitorException
    {
        ...
        hyperv.connect(physicalMachineAddress, pm.getUsername(), pm.getPassword());

        // Get concrete virtual machine state
        VMEventType state = VMEventType.UNKNOWN;
        try
        {
            int stateValue = hyperv.getState(virtualMachineName);
            state = HyperVUtils.translateEvent(stateValue);
        }
        finally
        {
            ...
            synchronized (monitoredMachines)
            {
                for (String physicalMachineAddress : monitoredMachines)
                {
                    ...
                    try
                    {
                        ...
                        for (MsvmComputerSystem vm : vms)
                        {
                            // Save the VM in the list of current VMs
                            String vmName = vm.getElementName();
