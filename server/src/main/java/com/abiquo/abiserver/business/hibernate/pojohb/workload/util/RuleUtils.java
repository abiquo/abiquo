...
package com.abiquo.abiserver.business.hibernate.pojohb.workload.util;
...
import com.abiquo.abiserver.pojo.workload.PersistentRule;
...
public class RuleUtils
{
    ...
    private static class RuleComparator implements Comparator<PersistentRule>, Serializable
    {
        ...
        {
            if (rule instanceof FitPolicyRuleHB)
            {
                return 0;
            }
            else if (rule instanceof EnterpriseExclusionRuleHB)
            {
                return 1;
            }
            else if (rule instanceof MachineLoadRuleHB)
            {
                return 2;
            }
            else
