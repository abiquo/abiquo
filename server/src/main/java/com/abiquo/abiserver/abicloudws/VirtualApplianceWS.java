...
public class VirtualApplianceWS implements IVirtualApplianceWS {
	...
	private final AbiConfig abiConfig = AbiConfigManager.getInstance()
			.getAbiConfig();

	private static final ResourceManager resourceManager = new ResourceManager(
			VirtualApplianceWS.class);
	...
	private final static OVFSerializer ovfSerializer = OVFSerializer
			.getInstance();
	...
	public VirtualApplianceWS() {
		try {
			infrastructureWS = (IInfrastructureWS) Thread
					.currentThread()
					.getContextClassLoader()
					.loadClass(
							"com.abiquo.abiserver.abicloudws.InfrastructureWSPremium")
					.newInstance();
		} catch (Exception e) {
	 ...
	 * 
	 * @see
	 * com.abiquo.abiserver.abicloudws.IVirtualApplianceWS#startVirtualAppliance
	 * (com.abiquo.abiserver .pojo.virtualappliance.VirtualAppliance)
	 */
	public BasicResult startVirtualAppliance(
		...
		try {
			EnvelopeType envelope = OVFModelFactory
					.createOVFModelFromVirtualAppliance()
					.createVirtualApplication(virtualAppliance);

			Document docEnvelope = ovfSerializer
					.bindToDocument(envelope, false); // TODO
			// notnamespaceaware

			String virtualSystemMonitor = RemoteServiceUtils
			...
			if (resource != null) {
				...
				errorManager.reportError(VirtualApplianceWS.resourceManager,
						result, "resourceNotFound", virtualAppliance.getName());
			}
		} catch (FaultException e) {
			encapsulateAndRethrowFault(virtualAppliance, e,
					EventType.VAPP_POWERON);
	 ...
	 * @throws Exception
	 */
	BasicResult forceCreateVirtualAppliance(
		...
		try {
			...
			String destination = RemoteServiceUtils
					.getVirtualFactoryFromVA(virtualAppliance);
			String virtualSystemMonitorAddress = RemoteServiceUtils
					.getVirtualSystemMonitorFromVA(virtualAppliance);
			long timeout = abiConfig.getTimeout();
			...
			encapsulateAndRethrowFault(
					virtualAppliance,
					e,
					EventType.VAPP_REFRESH,
	 ...
	 */
	private BasicResult changeState(final Resource resource,
			final EnvelopeType envelope, final String machineState)
		...
		EnvelopeType envelopeRunning = OVFModelFactory
				.createOVFModelFromVirtualAppliance()
				.changeStateVirtualMachine(envelope, machineState);
		Document docEnvelopeRunning = ovfSerializer.bindToDocument(
				envelopeRunning, false); // TODO
		// notnamespaceaware
	 ...
	 */
	private BasicResult changeState(final Resource resource,
			final EnvelopeType envelope, final VirtualAppliance virtualapp)
			throws ParserConfigurationException, JAXBException, SOAPException,
			IOException, FaultException, DatatypeConfigurationException,
			EmptyEnvelopeException, SectionException {
		BasicResult result = new BasicResult();
		...
		EnvelopeType envelopeRunning = OVFModelFactory
				.createOVFModelFromVirtualAppliance()
				.changeVirtualMachineStates(envelope, virtualapp);
		Document docEnvelopeRunning = ovfSerializer.bindToDocument(
				envelopeRunning, false); // TODO
		// notnamespaceaware
	 ...
	 * 
	 * @see
	 * com.abiquo.abiserver.abicloudws.IVirtualApplianceWS#shutdownVirtualAppliance
	 * (com.abiquo.abiserver .pojo.virtualappliance.VirtualAppliance, boolean)
	 */
	public BasicResult shutdownVirtualAppliance(
			final VirtualAppliance virtualAppliance) throws Exception {
		BasicResult result = null;
		result = new BasicResult();
			...
			if (resultCheck) {
				...
				if (resource != null) {
					...
					errorManager.reportError(
							VirtualApplianceWS.resourceManager, result,
							"resourceNotFound", virtualAppliance.getName());
				}
			} else {
				result
						.setMessage("The health check for this virtual appliance was not succesful");
				return result;
			}

		} catch (FaultException e) {
			encapsulateAndRethrowFault(virtualAppliance, e,
					EventType.VAPP_POWEROFF);
		} catch (RemoteServiceException e) {
			errorManager.reportError(VirtualApplianceWS.resourceManager,
					result, "virtualFactoryError", e, virtualAppliance
							.getName());
	...
	public BasicResult removeNodes(List<Node> nodesToDelete)
			throws PersistenceException, JAXBException,
			ParserConfigurationException, SOAPException, IOException,
			FaultException, DatatypeConfigurationException,
			VirtualFactoryHealthException {
		...
		for (Node nodeToDelete : nodesToDelete) {
			if (nodeToDelete.isNodeTypeVirtualImage()) {
				...
				StateEnum vmState = nodeVi.getVirtualMachine().getState()
						.toEnum();
					BasicResult removeResult = infrastructureWS
							.removeVirtualSystem(nodeVi.getVirtualMachine());
		...
		if (!checkOK) {
			throw new VirtualFactoryHealthException(
					"The virtual machine health did not pass");
		}
	 ...
	 * 
	 * @see
	 * com.abiquo.abiserver.abicloudws.IVirtualApplianceWS#deleteVirtualAppliance
	 * (com.abiquo.abiserver .pojo.virtualappliance.VirtualAppliance)
	 */
	public BasicResult deleteVirtualAppliance(
			final VirtualAppliance virtualAppliance) throws Exception {

		BasicResult result = new BasicResult();

		EnvelopeType envelope = OVFModelFactory
				.createOVFModelFromVirtualAppliance().createVirtualApplication(
	 ...
	 * 
	 * @param virtualAppliance
	 *            the virtualAppliance to find the resource from
	 ...
	 */
	private Resource findResource(final VirtualAppliance virtualAppliance)
			throws SOAPException, JAXBException, IOException, FaultException,
			DatatypeConfigurationException, PersistenceException,
			RemoteServiceException {
		/**
		 * TODO duplicated on InfrastructureWS
		 */
		// Creating a selector passing as the id the machine name
		SelectorSetType selector = createSelectorId(String
				.valueOf(virtualAppliance.getId()));
		Resource[] resources = ResourceFactory.find(RemoteServiceUtils
				.getVirtualFactoryFromVA(virtualAppliance),
				AbiCloudConstants.RESOURCE_URI, abiConfig.getTimeout(),
				selector);
	 ...
	 * 
	 * @see
	 * com.abiquo.abiserver.abicloudws.IVirtualApplianceWS#editVirtualAppliance
	 * (com.abiquo.abiserver .pojo.virtualappliance.VirtualAppliance)
	 */
	public BasicResult editVirtualAppliance(
			final VirtualAppliance virtualAppliance) throws Exception {
		BasicResult result = new BasicResult();
		result.setSuccess(true);
		...
		try {
			if (virtualAppliance.getState().toEnum() == StateEnum.APPLY_CHANGES_NEEDED) {
				...
				if (virtualAppliance.getNodes().size() > 0) {
					virtualAppliance
							.setState(new State(StateEnum.NOT_DEPLOYED));
	 ...
	 * 
	 * @see
	 * com.abiquo.abiserver.abicloudws.IVirtualApplianceWS#checkVirtualAppliance
	 * (com.abiquo.abiserver .pojo.virtualappliance.VirtualAppliance, boolean)
	 */
	public Boolean checkVirtualAppliance(final VirtualAppliance virtualAppliance)
			throws Exception {
		logger
				.info("Checking the Virtual Appliance health in community version is not done");
	...
	public Boolean checkRemovedNodes(final VirtualAppliance virtualAppliance)
		...
		for (Node node : nodes) {
			if (node.isNodeTypeVirtualImage()) {
				...
				if (nodeVi.getModified() == Node.NODE_ERASED) {
					logger
							.info("Checking Virtual machine before the VM operation");
					Boolean checkResult = infrastructureWS
							.checkVirtualSystem(nodeVi.getVirtualMachine());
					if (!checkResult) {
		...
		if (!checkTotalResult) {
			throw new VirtualFactoryHealthException(
					"The nodes removed don't pass the health check");
		}
	...
	/**
	 * Invokes a pull subscribe to the virtual system monitor to forces the
	 * events refreshing
	 * 
	 * @param virtualAppliance
	 *            the virtual appliance to be refreshed
	 * @return a basicResult with the resulting operation
	 */
	public BasicResult forceRefreshVirtualApplianceState(
			final VirtualAppliance virtualAppliance) {
		logger.info("Refreshing the virtual appliance state: {}",
	 ...
	 * @param virtualAppliance
	 */
	public void rollbackEventSubscription(
			final VirtualAppliance virtualAppliance) {
		EventingSupport.unsubscribeToAllVA(virtualAppliance);
	}
	 ...
	 * 
	 * @see
	 * com.abiquo.abiserver.abicloudws.IVirtualApplianceWS#bundleVirtualAppliance
	 * (com.abiquo.abiserver .pojo.virtualappliance.VirtualAppliance)
	 */
	public BasicResult bundleVirtualAppliance(
			final VirtualAppliance virtualAppliance) throws Exception {
		BasicResult result = new BasicResult();
		...
		try {
			...
			if (resultCheck) {
				EnvelopeType envelope = OVFModelFactory
						.createOVFModelFromVirtualAppliance()
						.createVirtualApplication(virtualAppliance, true);

				Document docEnvelope = ovfSerializer.bindToDocument(envelope,
						false);
				Resource resource = findResource(virtualAppliance);
	 ...
	 * <p>
	 * This method parses {@link FaultException} to provide human-readable
	 * information about the failure.
	 * 
	 * @param va
	 *            The Virtual Appliance data.
	 * @param ex
	 ...
	 * @throws VirtualApplianceFaultException
	 *             The encapsulated exception.
	 */
	private void encapsulateAndRethrowFault(final VirtualAppliance va,
			final FaultException ex, final EventType event, String message)
			throws VirtualApplianceFaultException {
		String exceptionMessage = null;
		try {
			// Try to find the original exception details
			BufferedReader br = new BufferedReader(new StringReader(ex
					.getMessage()));
			...
			while (line != null) {
				...
				if (detailIndex != -1) {
					...
					exceptionMessage = line.substring(detailBegin + 1, line
							.length());
		...
		String logMessage = (message == null) ? exceptionMessage : message
	 ...
	 */
	private void encapsulateAndRethrowFault(final VirtualAppliance va,
			final FaultException ex, final EventType event)
			throws VirtualApplianceFaultException {
	 ...
	 * 
	 * @param va
	 *            The Virtual Appliance information.
	 * @param event
	 *            The event to trace.
	 * @param message
	 *            The message to trace.
	 */
	private void traceLog(final VirtualAppliance va, final EventType event,
			final String message) {
		NodeVirtualImage vi = (NodeVirtualImage) va.getNodes().iterator()
				.next();
		...
		com.abiquo.tracer.Rack tracerRack = com.abiquo.tracer.Rack.rack(rack
				.getName());
		tracerRack.setMachine(machine);

		Datacenter datacenter = Datacenter.datacenter(pm.getDataCenter()
				.getName());
		...
		VirtualDatacenter vdc = VirtualDatacenter.virtualDatacenter(va
				.getVirtualDataCenter().getName());
		vdc.setVirtualAppliance(com.abiquo.tracer.VirtualAppliance
				.virtualAppliance(va.getName()));
		platform.getEnterprise().setVirtualDatacenter(vdc);

		// Log to tracer the original message
		TracerFactory.getTracer().log(SeverityType.CRITICAL,
				ComponentType.VIRTUAL_APPLIANCE, event, message, platform);
	}
}
