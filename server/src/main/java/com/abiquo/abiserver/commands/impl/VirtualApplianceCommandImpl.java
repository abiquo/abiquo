...
public class VirtualApplianceCommandImpl extends BasicCommand implements VirtualApplianceCommand
{
        ...
        if (currentStateAndAllow.getSuccess())
        {
            ...
            catch (Exception e)
            {
                ...
                if (e instanceof VirtualApplianceTimeoutException)
                {
                    ...
                    traceLog(SeverityType.MAJOR, ComponentType.VIRTUAL_APPLIANCE,
                        EventType.VAPP_UNKNOWN, userSession, null, virtualAppliance
        ...
        HibernateException, NotEnoughResourcesException, VirtualImageException
    {
        ...
        // Unsuscribe to all VA. We perform this action in case one of the virtual machines are
        // already deployed
        // and Abiquo does not know it. The best way to avoid 'move' and 'delete' side effects is to
        // delete de
            ...
            for (Node node : nodes)
            {
                ...
                if (node.isNodeTypeVirtualImage())
                {
                    ...
                    if (virtualMachine != null)
                    {
                        ...
                        if (vmachineHB != null)
                        {
                            // If it is allocated, set it to undeployed or unknown depending on the
                            // timeout exception
                            vmachineHB.setState(virtualMachine.getAssignedTo() != null
                                && isTimeoutException ? StateEnum.UNKNOWN : StateEnum.NOT_DEPLOYED);

                            if (isTimeoutException)
                            {
                                timeoutMsg.append(vmachineHB.getName());
                                if (virtualMachine.getAssignedTo() != null)
                                {
                                    HyperVisor hv = (HyperVisor) virtualMachine.getAssignedTo();
                                    timeoutMsg.append(" at ").append(hv.getIp());
                                }
                                timeoutMsg.append("\n");
                            }
