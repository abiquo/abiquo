...
public class IpPoolManagementDAO extends
		DefaultDAOBase<Integer, IpPoolManagement> {
	...
	public static final String BY_ENT = " SELECT ip FROM IpPoolManagement ip "
			...
			+ " left join ip.virtualAppliance vapp, "
			+ " NetworkConfiguration nc, " + " VirtualDatacenter vdc, "
	...
	public List<IpPoolManagement> findByVirtualMachine(
			final VirtualMachine virtualMachine) {
		Criteria criteria = getSession().createCriteria(IpPoolManagement.class);

		Criterion onVM = Restrictions.eq(
				IpPoolManagement.VIRTUAL_MACHINE_PROPERTY, virtualMachine);
	 ...
	 * @return
	 */
	public List<IpPoolManagement> findByPrivateVLAN(final Integer vdcId,
			final Integer vlanId, Integer firstElem, final Integer numElem,
			final String has, final IpPoolManagement.OrderByEnum orderby,
		...
		Query finalQuery = getSession().createQuery(
				BY_VLAN + " " + defineFilterAvailable(available)
		...
		PagedList<IpPoolManagement> ipList = new PagedList<IpPoolManagement>(
				finalQuery.list());
	...
	public List<IpPoolManagement> findByVdc(final Integer vdcId,
			Integer firstElem, final Integer numElem, final String has,
			final IpPoolManagement.OrderByEnum orderby, final Boolean asc) {
		// Get the query that counts the total results.
		Query finalQuery = getSession().createQuery(
				BY_VDC + " " + defineOrderBy(orderby, asc));
		finalQuery.setParameter("vdc_id", vdcId);
		finalQuery.setParameter("filterLike", (has.isEmpty()) ? "%" : "%" + has
				+ "%");
	...
	public List<IpPoolManagement> findByEnterprise(Integer entId,
			Integer firstElem, final Integer numElem, final String has,
			final IpPoolManagement.OrderByEnum orderby, final Boolean asc) {
		// Get the query that counts the total results.
		Query finalQuery = getSession().createQuery(
				BY_ENT + " " + defineOrderBy(orderby, asc));
	...
	public List<IpPoolManagement> getNetworkPoolPurchasedByEnterprise(
			final Integer enterpriseId) {
		Query query = getSession().createQuery(
				GET_NETWORK_POOL_PURCHASED_BY_ENTERPRISE);
	...
	public List<IpPoolManagement> findByVirtualAppliance(VirtualAppliance vapp) {
		Criterion onVapp = Restrictions.eq(
				IpPoolManagement.VIRTUAL_APPLIANCE_PROPERTY, vapp);
		Criteria criteria = getSession().createCriteria(IpPoolManagement.class)
				.add(onVapp);
		List<IpPoolManagement> result = getResultList(criteria);
	 ...
	 * 
	 * @param available
	 *            boolean if we shoud insert the filter or not.
	 * @return the query string that defines the filter.
	 */
	private String defineFilterAvailable(Boolean available) {
	...
	private String defineOrderBy(IpPoolManagement.OrderByEnum orderBy,
			final Boolean asc) {
