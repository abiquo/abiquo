...
void iniparser_dumpfields(dictionary * d, FILE * f)
{
    int i;
...
dictionary * iniparser_merge(dictionary * source, dictionary * working)
{
    int i;
    ...
    for (i = 0; i < source->size; i++)
    {
        ...
        if (source->val[i] != NULL)
        {
            iniparser_setstring(working, source->key[i], source->val[i]);
        } 
    }
