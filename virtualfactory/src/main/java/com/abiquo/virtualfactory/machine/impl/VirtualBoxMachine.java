...
public class VirtualBoxMachine extends AbsVirtualMachine {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(VirtualBoxMachine.class);
	 ...
	 */
	public VirtualBoxMachine(VirtualMachineConfiguration config)
			throws VirtualMachineException {
		super(config);

		if (config.isSetHypervisor()
				&& config.getHyper() instanceof VirtualBoxHypervisor) {
			vBoxHyper = (VirtualBoxHypervisor) config.getHyper();
		} else {
			throw new VirtualMachineException(
					"VirtualBoxMachiner requires a VirtualBoxHypervisor "
	...
	public void deployMachine() throws VirtualMachineException {
		...
		logger.info("Created virtualbox machine name:"
				+ config.getMachineName() + "\t ID:"
				+ config.getMachineId().toString() + "\t "
				+ "using hypervisor connection at "
	...
	/**
	 * Private helper to create a virtual machine template from the open
	 * virtualization format parameters.
	 * 
	 * @throws Exception
	 */
	private void createVirtualMachine() {
		...
		machine = vbox.createMachine(null, config.getMachineName(), "Other",
				config.getMachineId().toString(), Boolean.TRUE);
	...
	private void configureVirtualDiskResources() throws VirtualMachineException {
		...
		IStorageController controller = machine.addStorageController(
				sataStorageControllerName, StorageBus.SATA);

		// Adding the scsci controller

		/**
		 * Setting the I/O cache prevents to the virtual machine process to hang
		 * when power off.
		 * <p>
		 * http://www.virtualbox.org/ticket/8276
		 * </p>
		 * (12:13:44 PM) klaus-vb: apuig: the workaround is disabling async I/O
		 * for the controller, by ticking "use host I/O cache" for the scsi
		 * controller in the VM config.
		 */
		machine
				.addStorageController(scsiStorageControllerName,
						StorageBus.SCSI).setUseHostIOCache(true);

		if (config.getVirtualDiskBase().getDiskType() == VirtualDiskType.STANDARD) {
			// Attaching base disks
			logger.debug("Attaching the harddisk with the id: {}", newVDI
					.getId());

			machine.attachDevice(scsiStorageControllerName, 0, 0,
					DeviceType.HardDisk, newVDI);
			...
			attachIscsiDisk(config.getVirtualDiskBase(), vbox, machine,
					scsiStorageControllerName);
		}
	 ...
	 * 
	 * @param vnicList
	 *            the mac list with the interface bridge list to attach
	 * @throws VirtualMachineException
	 */
	private void attachBridgeInterfaces(List<VirtualNIC> vnicList)
			throws VirtualMachineException {
		try {
			String abiquoPrefix = AbiCloudModel.getInstance()
					.getConfigManager().getConfiguration().getBridgePrefix();
			for (VirtualNIC virtualNIC : vnicList) {
				String bridgeName = abiquoPrefix + "_"
						+ virtualNIC.getVlanTag();

				// Creating the VLAN
				URL phymach_ip = vBoxHyper.getAddress();

				URL aimURL = new URL(phymach_ip.getProtocol(), phymach_ip
						.getHost(), 8889, phymach_ip.getFile());

				VlanStub
						.createVlan(aimURL, String.valueOf(virtualNIC
								.getVlanTag()), virtualNIC.getVSwitchName(),
								bridgeName);

				attachNetworkAdapter(machine, virtualNIC.getMacAddress(),
						abiquoPrefix + "_" + virtualNIC.getVlanTag(), vnicList
								.indexOf(virtualNIC));
			}
		} catch (VLANException e) {
			throw new VirtualMachineException(
					"An error was occurred when configuring the networking for virtual machine :"
							+ machineName, e);
	 ...
	 * @throws VirtualMachineException
	 */
	private void detachBridgeInterfaces(List<VirtualNIC> vnicList)
			throws VirtualMachineException {
		try {
			String abiquoPrefix = AbiCloudModel.getInstance()
					.getConfigManager().getConfiguration().getBridgePrefix();
			for (VirtualNIC virtualNIC : vnicList) {
				...
				if (mustDeleteVLAN(bridgeName)) {
					VlanStub.deleteVlan(aimURL, String.valueOf(virtualNIC
							.getVlanTag()), virtualNIC.getVSwitchName(),
							bridgeName);
	...
	/**
	 * Checks in all virtual machines in the hypervisor to check if the bridge
	 * name is in used. If is in used the bridge name tagged with the VLAN tag
	 * cann't be deleted
	 * 
	 * @param bridgeName
	 *            the bridge name
	...
	private boolean mustDeleteVLAN(String bridgeName) {
		...
		long networkAdapterCount = vBoxHyper.getVirtualBox()
	...
	/**
	 * Private helper to attach the network adapter with the MAC object
	 * information.
	 * 
	 * @param targetMachine
	 *            the target machine
	 ...
	 * @param slot
	 */
	private void attachNetworkAdapter(IMachine targetMachine,
			String macAddress, String bridgeName, int slot) {
		// Attaching the network adapter
		INetworkAdapter networkAdapter = machine.getNetworkAdapter(new Long(
				slot));
	...
	/**
	 * Private helper to attach the virtual extended disks from a virtual
	 ...
	 *            the vbox
	 * @param machine
	 *            the machine to attache the disks
	 * @throws VirtualMachineException
	 */
	private void attachExtendedDisks(List<VirtualDisk> list, IVirtualBox vbox,
			IMachine machine) throws VirtualMachineException {
		if (!list.isEmpty()) {
			for (VirtualDisk vdisk : list) {
				...
				if (vdisk.getDiskType().compareTo(VirtualDiskType.ISCSI) == 0) {
					if (lastControllerPortUsed <= maxDevicesPerPortCount) {
						attachIscsiDisk(vdisk, vbox, machine,
								sataStorageControllerName);
					} else {
						throw new VirtualMachineException(
								"The maximum devices to attach was reached. Impossible to add more extended disks");
					}
				}
			}
		}
	 ...
	 * 
	 * @param vdisk
	 *            the virtual disk to attach
	 * @param vbox
	 *            the {@link IVirtualBox} object used to create the hard disk
	 * @param machine
	 *            the machine to attach the hard disk
	 * @param controllerName
	 *            the controller name
	 */
	private void attachIscsiDisk(VirtualDisk vdisk, IVirtualBox vbox,
			IMachine machine, String controllerName) {
		...
		iscsidiskVDI.setProperty("InitiatorName",
		...
		machine.attachDevice(controllerName, lastControllerPortUsed, 0,
				DeviceType.HardDisk, iscsidiskVDI);
	...
	protected void cloneVirtualDisk() throws Exception {
		...
		logger.info("Assigning the virtual disk [{}] from repository[{}]",
				destinationPath, destinationRepository);
	 ...
	 * 
	 * @param imageRemotePath
	 *            the image remote path
	 * @param clonedImagePath
	 *            the cloned image path
	 * @throws VirtualMachineException
	 *             the virtual machine exception
	 */
	public void cloneThroughAPI(String imageRemotePath, String clonedImagePath)
		...
		try {
			...
			diskVDI = vbox.openMedium(imageRemotePath, DeviceType.HardDisk,
			...
			String msg = "Can not open the HardDisk on [{}] (now try to find on media registry) caused by [{}]";
			logger.warn(msg, imageRemotePath, wse);

			try {
				diskVDI = vbox.findMedium(imageRemotePath, DeviceType.HardDisk);
			} catch (javax.xml.ws.WebServiceException wse2) {
				String msg2 = "Can not open of find on media registry the HardDisk ["
						+ imageRemotePath + "]";
		...
		newVDI = vBoxHyper.getVirtualBox().createHardDisk(diskVDI.getFormat(),
				clonedImagePath);
		IProgress progress = diskVDI
				.cloneTo(newVDI, diskVDI.getVariant(), null);
	...
	private void openRemoteSession() throws VirtualMachineException {
		...
		IProgress oProgress = machine.launchVMProcess(oSession, sessionType,
				env);
		logger.info("Session for VM " + config.getMachineId().toString()
				+ " is opened...");
		...
		try {
			...
			logger
					.error("An error waiting the session to be closed was occurred: "
							+ e.getMessage());
		}
	...
	public void powerOnMachine() throws VirtualMachineException {
		if (!checkState(State.POWER_UP)) {
			...
			if (AddressingUtils.isValidPort(String.valueOf(rdpPort))) {
				...
				vrdpserver
						.setVRDEProperty("TCP/Ports", String.valueOf(rdpPort));
			} else {
	 ...
	 */
	private void waitOperation(IProgress progress, long totalms)
			throws VirtualMachineException {
		for (long current = 0; current < totalms; current = current + 1000) {
			try {
				...
				if (progress.getCompleted()) {
					if (progress.getResultCode() != 0) {
						throw new VirtualMachineException(progress
								.getErrorInfo().getText());
					}
			...
			logger.debug("Vbox op %s at %d",
					progress.getOperationDescription(), progress
							.getOperationPercent());
		}

		throw new VirtualMachineException(String.format(
				"Timeout [%s] it waits %d seconds", progress.getDescription(),
				totalms / 1000));
	...
	public void deleteMachine() throws VirtualMachineException {
		...
		try {
			...
			logger
					.error(
							"An error was occurred then deconfiguring the networking resources: {}",
							e1);
		}
		// Getting the virtualBox hypervisor
		IVirtualBox vbox = vBoxHyper.getVirtualBox();
		...
		List<IMedium> mediumsToDelete = machine
	...
	public State getStateInHypervisor() {
		try {
			...
			logger
					.error(
							"An error was occurred when getting the virtual machine state",
							e);
	 ...
	 * (non-Javadoc)
	 * 
	 * @seecom.abiquo.abicloud.model.AbsVirtualMachine#reconfigVM(com.abiquo.
	 * abicloud.model.config. VirtualMachineConfiguration)
	 */
	@Override
	public void reconfigVM(VirtualMachineConfiguration newConfiguration)
		...
		if (oSession.getState().compareTo(SessionState.Unlocked) == 0) {
			...
			if (newConfiguration.isRam_set()) {
				machine.setMemorySize(newConfiguration.getMemoryRAM()
						/ (1024 * 1024));
			}
	...
	public boolean isVMAlreadyCreated() throws VirtualMachineException {
		...
		try {
			...
			try {
				...
				logger.debug("The Virtual machine: {} does not exist", config
						.getMachineId().toString());
				return false;
			}
		}
		return true;
	 ...
	 * @param stateToCheck
	 *            the state to check
	 * @return true if the state in the hypervisors equals to the state as
	 *         parameter, false if contrary
	 */
	private boolean checkState(State stateToCheck)
			throws VirtualMachineException {
	...
	public void bundleVirtualMachine(String sourcePath, String destinationPath,
			String snapshotName, boolean isManaged)
			throws VirtualMachineException {

		logger.debug("Creating instance of image [{}] in clonedImagePath[{}]",
				sourcePath, destinationPath);
		logger.info("Creating an instance of the virtual machine: {}", config
				.getMachineName());
		try {
			...
			Iface aimclient = TTransportProxy.getInstance(hypervisorLocation,
					vBoxHyper.getAddress().getPort());
			aimclient.copyFromDatastoreToRepository(machineName, snapshotName,
			...
			throw new VirtualMachineException(
					"Fail to bundle the virtual machine", e);
		}
